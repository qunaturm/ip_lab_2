...
def main():
    ...
    imageWithNoise = filters.task1_noise(imageWithoutNoise, "images/result/manulNoise.png")


def task_1_noise(image):
    filtredImage = filters.add_gaussian_noise(image=image, mu=20, sigma=15)
    cv2.imwrite("result/task1/manulNoise.png", filtredImage)
    return filtredImage


def task_2_median_filter(image, path):
...
def task_3_cv2(image):
	...
	cv2.imwrite('resutl/task3/median_cv2.png', med_cv2)
